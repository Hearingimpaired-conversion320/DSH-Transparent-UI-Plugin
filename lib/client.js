window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*/
		function AquaPluginCard({ t, setEnabled, setBlur, setFrost, setFluidHue, useStore }) {
...
`;
		...
		*/
		...
		//#endregion
		//#region src/client/greetings.ts
		/**
		...
		*/
		...
		const lastPick = /* @__PURE__ */ new Map();
		...
		*/
		function pickGreeting(locale) {
			const buckets = locale === "zh" ? ZH_GREETINGS : EN_GREETINGS;
			const bucket = buckets[bucketOf(/* @__PURE__ */ new Date())] ?? [];
		...
		*/
		function resetHeroCopy(locale) {
			const greeting = BASE_GREETING[locale === "zh" ? "zh" : "en"];
			for (const headline of document.querySelectorAll("[data-hero-headline]")) headline.textContent = greeting;
			const textarea = document.querySelector("[data-phase=\"hero\"] textarea");
		...
		*/
		function aquaPlaceholder(locale) {
		...
		*/
		var AquaLayer = class {
			...
			*/
			decorate(root) {
				const headline = root.matches("[data-hero-headline]") ? root : root.querySelector("[data-hero-headline]");
			...
			startObserver() {
				const observer = new MutationObserver((records) => {
					for (const record of records) for (const node of record.addedNodes) if (node instanceof Element) this.decorate(node);
				});
